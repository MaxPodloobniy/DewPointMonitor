...
// Функція для обчислення точки роси
...
int main(void)
{
  ...
  {
	  if (DHT11_Start()) {
	              uint8_t RHI = DHT11_Read(); // Відносна вологість, ціла частина
	              uint8_t RHD = DHT11_Read(); // Відносна вологість, десята частина
	              uint8_t TCI = DHT11_Read(); // Температура, ціла частина
	              uint8_t TCD = DHT11_Read(); // Температура, десята частина
	              uint8_t SUM = DHT11_Read(); // Контрольна сума

	              if (RHI + RHD + TCI + TCD == SUM) {
	                  float tCelsius = (float)TCI + (float)(TCD / 10.0);
	                  float RH = (float)RHI + (float)(RHD / 10.0);
	                  float dew_point = calculate_dew_point(tCelsius, RH);
	                  ...
	                  lcd_send_cmd(0xC0);
	                  snprintf(buffer, sizeof(buffer), "Humidity: %d%%", (int)RH);
	                  lcd_send_string(buffer);

	                  // Перевірка, чи досягнута точка роси
	                  if (tCelsius <= dew_point) {
	                      ...
	                      for (int i = 0; i < 10; i++) {
	                          ...
	                          HAL_Delay(100);
	                          HAL_GPIO_WritePin(Zummer_pin_GPIO_Port, Zummer_pin_Pin, GPIO_PIN_RESET);
	                          HAL_Delay(100);
	                      }
	                  }

	                  // Затримка перед наступним циклом зчитування
	                  HAL_Delay(2000);
	              } else {
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  /*Configure GPIO pin Output Level */
  HAL_GPIO_WritePin(GPIOB, GPIO_PIN_13|Zummer_pin_Pin, GPIO_PIN_RESET);
  ...
  /*Configure GPIO pins : PB13 Zummer_pin_Pin */
  GPIO_InitStruct.Pin = GPIO_PIN_13|Zummer_pin_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
